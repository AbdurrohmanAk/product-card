...
const listFilms = ["Зеленная миля", "Побег из Шоушенка", "Форрест Гамп", "Интерстеллар"];
console.log(listFilms.includes('Интерстеллар'));
...
console.log(getReverseArray(listFilms));
...
const updateUser = userComments.map(comment => ({...comment, postId: comment.id <= 5 ? 2 : 1}));
console.log(updateUser)

// 9. Перебрать массив чтобы объекты сотсояли только из айди и имени 

const idNameArray = userComments.map(comment => ({id: comment.id, name: comment.name}));
...
const checkUserComments = userComments.map(comment => ({...comment, isInvalid: comment.body.length > 180}));
...
const getNewMail = userComments.reduce((acc, comment) => {acc.push(comment.email);
    return acc;}, [])
console.log(userMail)
console.log(getNewMail)
